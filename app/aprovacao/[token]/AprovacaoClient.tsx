...
/**
 * app/aprovacao/[token]/AprovacaoClient.tsx
 *
 * Componente client da página pública de aprovação.
 * Recebe os dados iniciais do Server Component e gerencia toda a interação.
 */
...
const ACTION_CONFIG = {
  aprovado:  { label: 'APROVAR',             icon: '✅', bg: 'bg-green-500 hover:bg-green-600',  border: 'border-green-600' },
  correcao:  { label: 'SOLICITAR CORREÇÃO',  icon: '✏️', bg: 'bg-amber-500 hover:bg-amber-600',  border: 'border-amber-600' },
  rejeitado: { label: 'REJEITAR',            icon: '❌', bg: 'bg-red-500   hover:bg-red-600',    border: 'border-red-600'   },
} as const;
...
function ConfirmationScreen({ status }: { status: ApprovalStatus }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center px-4">
      <div className="text-7xl mb-6 drop-shadow-sm">{cfg.icon}</div>
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
      ...
      <div className="relative bg-black">
        ...
        {/* Setas de navegação (em cima da mídia) */}
        {slides.length > 1 && (
          <>
            <button
...
interface AprovacaoClientProps {
  token:           string;
...
export default function AprovacaoClient({ token, initialApproval }: AprovacaoClientProps) {
  ...
  const [error, setError] = useState<string | null>(
    !initialApproval
      ...
      : isExpired(initialApproval.expiresAt as unknown as string)
      ...
      <main className="max-w-lg mx-auto px-4 py-6 pb-12">
        ...
        {responded && approval ? (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm flex items-center gap-3">
              ...
              {approval.createdAt && (
                <p className="text-[11px] text-gray-400 shrink-0 hidden sm:block">
                  {formatDateBR(approval.createdAt as unknown as string)}
                </p>
              )}
            </div>
            ...
            {/* Tags (se existirem) */}
            {(approval as unknown as { tags?: string[] }).tags?.length ? (
              <div className="flex gap-2 flex-wrap">
                {(approval as unknown as { tags: string[] }).tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-[11px] bg-purple-50 text-purple-700 border border-purple-200 px-2.5 py-1 rounded-full font-medium"
                  >
                    #{tag}
                  </span>
                ))}
            ...
            {approval.expiresAt && (
              <p className="text-center text-[11px] text-gray-400 pt-1">
                ⏰ Link válido até {formatDateBR(approval.expiresAt as unknown as string)}
              </p>
            )}
          </div>
        ) : null}
    ...
    >
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-inner">
          <span className="font-black text-white text-xl tracking-tight">A</span>
        </div>
        <div>
