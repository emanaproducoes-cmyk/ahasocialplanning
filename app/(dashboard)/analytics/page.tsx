...
const KPI_META: Record<KpiKey, { label: string; icon: string; color: string; borderStyle?: 'full' | 'black' }> = {
  curtidas:          { label: 'CURTIDAS',          icon: '♡',  color: '#F472B6' },
  comentarios:       { label: 'COMENTÁRIOS',        icon: '💬', color: '#A78BFA' },
  compartilhamentos: { label: 'COMPARTILHAMENTOS',  icon: '↗',  color: '#60A5FA' },
  envios:            { label: 'ENVIOS',             icon: '✉',  color: '#FCD34D' },
  mensagens:         { label: 'MENSAGENS',          icon: '💬', color: '#34D399' },
  mencoes:           { label: 'MENÇÕES',            icon: '@',  color: '#22D3EE' },
  remix:             { label: 'REMIX',              icon: '↺',  color: '#F97316', borderStyle: 'full' },
  salvamentos:       { label: 'SALVAMENTOS',        icon: '🔖', color: '#C084FC' },
  alcance:           { label: 'ALCANCE',            icon: '👁', color: '#38BDF8' },
  impressoes:        { label: 'IMPRESSÕES',         icon: '↗',  color: '#000000', borderStyle: 'black' },
};

// All 10 KPIs always shown in 2×5 grid
...
// Geographic data
const GEO_DATA = [
  { city: 'São Paulo, SP',      pct: 28.4, eng: 7.6, peak: '20h', cpm: 11,     platforms: { Instagram: 10, Facebook: 31, YouTube: 20, TikTok: 23, LinkedIn: 15 }, color: '#FF5C00' },
...
function KpiSparkline({ color, positive, kpiKey }: { color: string; positive: boolean; kpiKey: KpiKey }) {
  // Generate deterministic-ish sparkline per KPI
  const seed = kpiKey.length * 7;
  const points = Array.from({ length: 16 }, (_, i) => {
    const base = 20 + Math.sin((i + seed) * 0.6) * 12 + Math.cos((i + seed) * 0.3) * 8;
    return Math.max(8, Math.min(48, base + (i / 15) * (positive ? 14 : -8)));
  });
  const max = Math.max(...points);
  const min = Math.min(...points);
  const range = max - min || 1;
  const norm = points.map(p => ((p - min) / range) * 36 + 4);
  const w = 100;
  const stepX = w / (points.length - 1);
  const path = norm.map((y, i) => `${i === 0 ? 'M' : 'L'}${(i * stepX).toFixed(1)},${(44 - y).toFixed(1)}`).join(' ');
  const area = `${path} L${w},48 L0,48 Z`;
  const gradId = `spark-${kpiKey}`;

  return (
    <svg viewBox={`0 0 ${w} 48`} className="w-full" style={{ height: 48 }} preserveAspectRatio="none">
      <defs>
...
function KpiCard({
  ...
  const { label, icon, color, borderStyle } = KPI_META[kpiKey];
  const positive = variation >= 0;

  // Border style variants
  const isFullBorder = borderStyle === 'full';   // REMIX — all sides colored
  const isBlackTop   = borderStyle === 'black';  // IMPRESSÕES — black top

  const topBorderColor = isBlackTop ? '#111111' : color;

  const cardStyle: React.CSSProperties = isFullBorder
    ? {
    ...
    : {
        ...
        borderTop: `3.5px solid ${topBorderColor}`,
      };

  return (
    <button
      onClick={onClick}
      className="bg-white rounded-xl shadow-sm flex flex-col text-left hover:shadow-md transition-all group cursor-pointer overflow-hidden"
      style={cardStyle}
    >
      {/* Top row: label + icon */}
      <div className="flex items-center justify-between px-4 pt-3.5 pb-0">
        ...
        <span
          className="text-[15px] opacity-50 group-hover:opacity-80 transition-opacity"
          style={{ color }}
        >
          {icon}
        </span>
      ...
      <div className="px-4 pt-2 pb-1">
        <p
          className="text-[28px] font-black text-gray-900 leading-none tracking-tight"
      ...
      <div className="mt-auto w-full" style={{ lineHeight: 0 }}>
        <KpiSparkline color={isBlackTop ? '#111111' : color} positive={positive} kpiKey={kpiKey} />
      </div>
    </button>
  );
}
                ...
                <div key={plat} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${(pct / maxPlatform) * 100}%`, background: platformColors[plat] ?? '#FF5C00' }}
                    />
                  </div>
              ...
              <div key={group.range} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full flex flex-col items-center gap-0.5 justify-end" style={{ height: '140px' }}>
                  <button
                    ...
                    style={{ height: `${(group.feminino / maxAge) * 120}px`, background: '#FF5C00', minHeight: 4 }}
                    title={`Feminino ${group.feminino}%`}
                  />
                  <button
                    onClick={() => onAgeClick(group.range)}
                    className="w-full hover:opacity-80 transition-opacity"
                    style={{ height: `${(group.masculino / maxAge) * 120}px`, background: '#7C3AED', minHeight: 4 }}
...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-5 animate-fade-in pb-10">
      ...
      {/* ── KPIs — 2 rows × 5 cols, all 10 always shown ── */}
      {sections.kpis && (
        <div>
          {/* Section header row */}
          <div className="flex items-center justify-between mb-3">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          >
            <div className="flex items-center gap-3">
              <div className="w-7 h-7 rounded-full flex items-center justify-center shrink-0" style={{ background: '#FF5C00' }}>
