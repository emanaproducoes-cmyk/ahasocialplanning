...
function ImageViewer({ post }: { post: Post }) {
  ...
  return (
    <>
      <div className="relative rounded-xl overflow-hidden mb-4 bg-black cursor-zoom-in" onClick={() => setLightboxOpen(true)}>
        {isVideo
          ? <video src={current} controls className="w-full max-h-[340px] object-contain" />
          // eslint-disable-next-line @next/next/no-img-element
      ...
      {lightboxOpen && <MediaLightbox slides={slides} initialSlide={slide} postTitle={post.title} onClose={() => setLightboxOpen(false)} />}
    </>
  );
}
