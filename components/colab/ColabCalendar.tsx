...
export default function ColabCalendar({ session }: { session: ColabSession }) {
    ...
    try {
      ...
      const data = snap.docs.map(d => {
        ...
        return {
          ...
          scheduledAt: raw.scheduledAt?.toDate?.().toISOString?.() ?? raw.scheduledAt,
          creatives: raw.creatives,
          mediaUrl: raw.creatives?.[0]?.url ?? raw.mediaUrl ?? raw.fileUrl,
          fileType: (raw.creatives?.[0]?.type ?? '').includes('video') ? 'video' : 'image',
  ...
  const postsByDay = days.reduce((acc, day) => {
    ...
    acc[key] = posts.filter(p => {
      if (!p.scheduledAt) return false;
      try { return isSameDay(new Date(p.scheduledAt), day); } catch { return false; }
    });
    return acc;
  ...
  const k = days.slice(0, 1).sort((a, b) => (a.scheduledAt ?? '').localeCompare(b.scheduledAt ?? ''));
    ...
    <div style={{ padding: '24px 28px', background: 'linear-gradient(135deg, #EDE9FE 0%, #EEF2FF 50%, #F0F9FF 100%)', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {/* View toggle */}
          <div style={{ display: 'flex', gap: 4, padding: '4px', background: 'rgba(255,255,255,0.6)', borderRadius: 10, border: '1px solid rgba(255,255,255,0.8)', backdropFilter: 'blur(8px)' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.70)', borderRadius: 20, border: '1px solid rgba(255,255,255,0.8)', overflow: 'hidden', backdropFilter: 'blur(16px)', boxShadow: '0 8px 32px rgba(109,40,217,0.12)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 18px', borderBottom: '1px solid rgba(139,92,246,0.15)', background: 'linear-gradient(135deg, rgba(139,92,246,0.12), rgba(99,102,241,0.08))' }}>
            <button onClick={() => setMonth(m => subMonths(m, 1))}
              style={{ background: 'rgba(139,92,246,0.15)', border: '1px solid rgba(139,92,246,0.25)', cursor: 'pointer', color: '#7C3AED', fontSize: 16, padding: '4px 10px', borderRadius: 8, fontWeight: 700, backdropFilter: 'blur(4px)' }}>‹</button>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', borderBottom: '1px solid rgba(139,92,246,0.12)', background: 'rgba(245,243,255,0.60)' }}>
            {['DOM','SEG','TER','QUA','QUI','SEX','SÁB'].map(d => (
              <div key={d} style={{ padding: '8px 6px', textAlign: 'center', fontSize: 10, fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, color: '#7C3AED', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                {d}
              </div>
            ))}
          </div>
          {/* Grid */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', border: '1px solid rgba(255,255,255,0.4)', borderRadius: 0, overflow: 'hidden', background: 'rgba(255,255,255,0.20)' }}>
            {days.map((day, idx) => {
              const key        = format(day, 'yyyy-MM-dd');
              ...
              return (
                <div key={key}
                  ...
                  style={{
                    ...
                    borderRight: (idx + 1) % 7 !== 0 ? '1px solid rgba(255,255,255,0.5)' : 'none',
                    outline: isT ? '2px solid #7C3AED' : 'none',
                    outlineOffset: '-2px',
                    boxShadow: isT ? '0 0 0 2px rgba(124,58,237,0.3), inset 0 0 12px rgba(124,58,237,0.08)' : 'none',
                    background: isT ? 'rgba(124,58,237,0.15)' : (dayPosts.length > 0 ? 'linear-gradient(135deg,rgba(139,92,246,0.12),rgba(99,102,241,0.08))' : 'rgba(255,255,255,0.60)'),
                    opacity: inMonth ? 1 : 0.35,
                    cursor: inMonth ? 'pointer' : 'default',
                    transition: 'background 0.15s',
                  }}
                  onMouseEnter={e => inMonth && (e.currentTarget.style.background = isT ? 'rgba(124,58,237,0.22)' : 'rgba(139,92,246,0.10)')}
                  onMouseLeave={e => (e.currentTarget.style.background = isT ? 'rgba(124,58,237,0.15)' : (dayPosts.length > 0 ? 'linear-gradient(135deg,rgba(139,92,246,0.12),rgba(99,102,241,0.08))' : 'rgba(255,255,255,0.60)'))}
                >
                  <div style={{
                    fontSize: 12, fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700,
                    color: isT ? '#7C3AED' : inMonth ? '#3B0764' : '#CBD5E1',
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    {dayPosts.slice(0, 2).map(p => {
                      ...
                      return (
                        <div key={p.id}
                          ...
                          style={{ borderRadius: 6, overflow: 'hidden', cursor: 'pointer', border: `1px solid ${cfg.dot}60`, boxShadow: '0 2px 8px rgba(109,40,217,0.10)', backdropFilter: 'blur(4px)', background: 'rgba(255,255,255,0.85)' }}>
                    ...
                    {dayPosts.length > 2 && (
                      <div style={{ fontSize: 9, color: '#7C3AED', fontWeight: 700, textAlign: 'right', paddingRight: 2, background: 'rgba(255,255,255,0.8)', borderRadius: 4, padding: '1px 4px' }}>
                        +{dayPosts.length - 2}
                      </div>
                    )}
                  </div>
            ...
            .map(p => {
              ...
              return (
                <div key={p.id} onClick={() => setSelected(p)} style={{
                  ...
                  backdropFilter: 'blur(8px)', boxShadow: '0 2px 12px rgba(109,40,217,0.08)',
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {p.scheduledAt && (
                      <div style={{ fontSize: 11, color: '#7C3AED', marginTop: 1, fontWeight: 600 }}>
                        📅 {format(new Date(p.scheduledAt), "dd 'de' MMM", { locale: ptBR })}
                      </div>
                    )}
                  </div>
      ...
      {selected && (
        <PostModal post={selected} session={session}
          ...
          onUpdated={() => { setSelected(null); reload(); }} onZoom={setZoomedImg}
...
function DayModal({ day, existingPosts, session, onClose, onSaved, onViewPost }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(59,7,100,0.55)', backdropFilter: 'blur(8px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      ...
      <div style={{ borderRadius: 24, width: '100%', maxWidth: 480, maxHeight: '88vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', background: 'rgba(255,255,255,0.90)', backdropFilter: 'blur(20px)', boxShadow: '0 20px 60px rgba(109,40,217,0.25), 0 0 0 1px rgba(255,255,255,0.5)' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '18px 22px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {existingPosts.map(p => {
                ...
                return (
                  <div key={p.id} onClick={() => onViewPost(p)}
                    style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderRadius: 12, background: 'rgba(245,243,255,0.8)', border: '1px solid rgba(139,92,246,0.20)', cursor: 'pointer', transition: 'all 0.12s' }}
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                {session.canCreate ? (
                  ...
                  <button onClick={() => setMode('request')} style={{ flex: 1, padding: '10px 0', borderRadius: 12, border: '1px solid rgba(139,92,246,0.30)', cursor: 'pointer', fontSize: 13, fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, background: 'rgba(245,243,255,0.8)', color: '#7C3AED' }}>📩 Solicitar post neste dia</button>
                )}
              </div>
            </div>
          )}
          ...
          {mode === 'request' && requested && (
            <div style={{ textAlign: 'center', padding: '32px 20px' }}>
              ...
              <p style={{ color: '#7C3AED', fontSize: 13, marginBottom: 20 }}>Sua agência foi notificada e criará o conteúdo para {format(day, "dd/MM", { locale: ptBR })}.</p>
...
function PostModal({ post, session, onClose, onUpdated, onDeleted, onZoom }: {
  ...
  const inputStyle = { width: '100%', padding: '10px 12px', borderRadius: 10, border: '1px solid rgba(139,92,246,0.25)', fontSize: 14, color: '#3B0764', background: 'rgba(245,243,255,0.6)', fontFamily: 'Inter, sans-serif', outline: 'none', boxSizing: 'border-box' as const };

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(59,7,100,0.55)', backdropFilter: 'blur(8px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
      onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={{ borderRadius: 24, width: '100%', maxWidth: 560, maxHeight: '92vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', background: 'rgba(255,255,255,0.92)', backdropFilter: 'blur(20px)', boxShadow: '0 20px 60px rgba(109,40,217,0.25), 0 0 0 1px rgba(255,255,255,0.5)' }}>
        {/* Header */}
        <div style={{ padding: '16px 20px 12px', borderBottom: '1px solid rgba(139,92,246,0.15)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', background: 'linear-gradient(135deg,rgba(139,92,246,0.10),rgba(99,102,241,0.06))' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
              {mediaUrl && (
                <div style={{ width: 32, height: 32, borderRadius: 6, overflow: 'hidden', flexShrink: 0 }}>
                  <img src={mediaUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onClick={() => onZoom(mediaUrl)} />
                </div>
              )}
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexShrink: 0 }}>
            <button onClick={() => setTab('actions')} style={{ background: 'rgba(139,92,246,0.12)', border: '1px solid rgba(139,92,246,0.25)', color: '#7C3AED', padding: '5px 10px', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 700, fontFamily: 'Plus Jakarta Sans, sans-serif', display: 'flex', alignItems: 'center', gap: 4 }}>✏️ Editar Post</button>
            <button onClick={handleDelete} disabled={deleting} style={{ background: '#FEF2F2', border: '1px solid #FECACA', color: '#B91C1C', padding: '5px 10px', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 700, fontFamily: 'Plus Jakarta Sans, sans-serif', display: 'flex', alignItems: 'center', gap: 4 }}>🗑️ Deletar</button>
        ...
        <div style={{ flex: 1, overflow: 'auto', background: 'rgba(255,255,255,0.80)' }}>
            ...
            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
              ...
              {post.caption && (
                <div style={{ width: '100%', maxWidth: 380 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: '#7C3AED', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>LEGENDA</div>
                  <p style={{ color: '#3B0764', fontSize: 14, lineHeight: 1.65, margin: 0 }}>{post.caption}</p>
                </div>
              )}
            </div>
          )}
                ...
                <div><div style={{ fontSize: 11, fontWeight: 700, color: '#7C3AED', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>PLATAFORMAS</div>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {post.platforms.map(pl => (
                      <div key={pl} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px', borderRadius: 8, background: 'rgba(245,243,255,0.8)', border: '1px solid rgba(139,92,246,0.20)', fontSize: 13, color: '#3B0764', fontWeight: 600, fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
          ...
          {tab === 'comments' && (
            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                ...
                <button onClick={submitComment} disabled={saving || !text.trim()} style={{ padding: '8px 14px', borderRadius: 10, border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#7C3AED,#4F46E5)', color: '#fff', alignSelf: 'flex-end', fontSize: 16, fontWeight: 700 }}>
                  {saving ? '…' : '➤'}
                </button>
              </div>
            </div>
          ...
          {tab === 'actions' && (
            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', paddingTop: 4 }}>
                <button onClick={onClose} style={{ padding: '9px 18px', borderRadius: 10, border: '1px solid rgba(139,92,246,0.25)', background: 'rgba(245,243,255,0.8)', color: '#7C3AED', cursor: 'pointer', fontSize: 13, fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 600 }}>Cancelar</button>
                <button onClick={handleSave} disabled={saving} style={{ padding: '9px 18px', borderRadius: 10, border: 'none', cursor: 'pointer', fontSize: 13, fontFamily: 'Plus Jakarta Sans, sans-serif', fontWeight: 700, background: 'linear-gradient(135deg,#7C3AED,#4F46E5)', color: '#fff' }}>
                  {saving ? '…' : '💾 Salvar'}
                </button>
              </div>
            </div>
