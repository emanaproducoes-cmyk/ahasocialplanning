...
const Icons = {
  ...
  analytics:   <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M3 3v18h18"/><path d="M7 16l4-4 4 4 4-8"/></svg>,
};
...
const NAV: { title: string; items: NavItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/analytics',    label: 'Analytics',    icon: Icons.analytics, isNew: true },
    ],
  },
];
...
function NavLink({ item, badges, collapsed, depth = 0 }: {
      ...
      <div>
        <button
          ...
          className={cn(
            ...
            isActive ? 'bg-[#FF5C00]/15 text-[#FF5C00]' : 'text-[#8B8BA7] hover:text-white hover:bg-white/5'
          )}
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        isActive ? 'bg-[#FF5C00]/15 text-[#FF5C00] font-semibold' : 'text-[#8B8BA7] hover:text-white hover:bg-white/5',
        collapsed && 'justify-center px-2',
        depth === 1 && 'py-1.5'
      )}
      style={{ fontFamily: "'Aileron', sans-serif" }}
    >
      <span className={cn('shrink-0', isActive ? 'opacity-100' : 'opacity-70')}>{item.icon}</span>
      {!collapsed && (
        <>
          <span className="flex-1 truncate">{item.label}</span>
          {item.isNew && (
            <span className="text-[10px] font-bold bg-[#FF5C00] text-white px-1.5 py-0.5 rounded-full shrink-0">NEW</span>
          )}
          {item.dotColor && <DotBadge color={item.dotColor} count={count} />}
          {!item.dotColor && item.badge && count > 0 && <DotBadge color="#FF5C00" count={count} />}
        </>
      )}
    </Link>
  );
      ...
      >
        ...
        <Link
          ...
          className={cn(
            ...
            collapsed ? 'justify-center py-4 px-2' : 'gap-3 px-4 py-3.5'
          )}
        >
          {/* Logo AHA — quadrado laranja com a imagem do logo (SEM texto "AHA" dentro) */}
          <div className="w-10 h-10 rounded-xl bg-[#FF5C00] flex items-center justify-center shrink-0 overflow-hidden">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src="/icons/logo_aha.png"
              alt="AHA"
          ...
          {!collapsed && (
            <div className="min-w-0">
              <p
                className="text-white text-[12px] font-bold leading-tight tracking-widest uppercase"
                style={{ fontFamily: "'Aileron', sans-serif", fontWeight: 700 }}
              >
                Social Planning
              </p>
              <p
                className="text-[#8B8BA7] text-[10px] leading-tight uppercase tracking-wider"
                style={{ fontFamily: "'Aileron', sans-serif" }}
              >
                Gestão de Conteúdo
              </p>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-5">
          {NAV.map(section => (
            <div key={section.title}>
              {!collapsed && (
                <p
                  className="text-[10px] font-bold tracking-[0.12em] text-[#8B8BA7]/60 px-3 mb-2 uppercase"
                  style={{ fontFamily: "'Aileron', sans-serif" }}
                >
                  {section.title}
                </p>
        ...
        <div className={cn('border-t border-white/8 shrink-0', collapsed ? 'p-2' : 'p-3')}>
          {user && !collapsed && (
            <div className="flex items-center gap-2.5 mb-2">
              ...
              <div className="min-w-0">
                <p className="text-white text-[13px] font-semibold truncate" style={{ fontFamily: "'Aileron', sans-serif" }}>
                  {user.displayName?.split(' ')[0] ?? 'Usuário'}
                </p>
                <p className="text-[#8B8BA7] text-[11px] truncate" style={{ fontFamily: "'Aileron', sans-serif" }}>
                  Gerente de Conteúdo
                </p>
              </div>
            </div>
